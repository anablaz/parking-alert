...
(function (e) {
  function t(t, r, s, o) {
    ...
    function N(t) {
      ...
      if (a.grid.tickColor == null)
        a.grid.tickColor = e.color
          .parse(a.grid.color)
          .scale("a", 0.22)
          .toString();
      for (n = 0; n < Math.max(1, a.xaxes.length); ++n)
        a.xaxes[n] = e.extend(true, {}, a.xaxis, a.xaxes[n]);
      for (n = 0; n < Math.max(1, a.yaxes.length); ++n)
        a.yaxes[n] = e.extend(true, {}, a.yaxis, a.yaxes[n]);
      if (a.xaxis.noTicks && a.xaxis.ticks == null)
        a.xaxis.ticks = a.xaxis.noTicks;
      if (a.yaxis.noTicks && a.yaxis.ticks == null)
      ...
      for (var r in E)
        if (a.hooks[r] && a.hooks[r].length) E[r] = E[r].concat(a.hooks[r]);
    ...
    function D() {
      ...
      for (t = 0; t < u.length; ++t) {
        ...
        if (s != null) {
          --n;
          if (typeof s == "number") i.push(s);
          else r.push(e.color.parse(u[t].color));
        }
      }
      ...
      while (o.length < n) {
        var l;
        if (a.colors.length == t) l = e.color.make(100, 100, 100);
        else l = e.color.parse(a.colors[t]);
        var c = f % 2 == 1 ? -1 : 1;
        l.scale("rgb", 1 + c * Math.ceil(f / 2) * 0.2);
        o.push(l);
        ++t;
    ...
    function F() {
      var n,
        r = t.children("canvas.base"),
        i = t.children("canvas.overlay");
      if (r.length == 0 || i == 0) {
        t.html("");
        t.css({ padding: 0 });
    ...
    function J(t) {
      var r = t.options;
      var i;
      if (typeof r.ticks == "number" && r.ticks > 0) i = r.ticks;
      else i = 0.3 * Math.sqrt(t.direction == "x" ? g : y);
      var s = (t.max - t.min) / i,
        o,
        u,
        a,
      ...
      if (r.mode == "time") {
        ...
        if (r.minTickSize != null) {
          if (typeof r.tickSize == "number") b = r.tickSize;
          else b = r.minTickSize[0] * p[r.minTickSize[1]];
        }
        for (var l = 0; l < m.length - 1; ++l)
          if (
            s < (m[l][0] * p[m[l][1]] + m[l + 1][0] * p[m[l + 1][1]]) / 2 &&
            m[l][0] * p[m[l][1]] >= b
          )
            break;
        o = m[l][0];
        ...
        f = function (t, n) {
          ...
          if (s < p.minute) fmt = "%h:%M:%S" + u;
          else if (s < p.day) {
            if (o < 2 * p.day) fmt = "%h:%M" + u;
            else fmt = "%b %d %h:%M" + u;
          } else if (s < p.month) fmt = "%b %d";
          else if (s < p.year) {
            if (o < p.year) fmt = "%b";
            else fmt = "%b %y";
          } else fmt = "%y";
      ...
      if (r.alignTicksWithAxis != null) {
        ...
        if (S && S.used && S != t) {
          ...
          if (x.length > 0) {
            ...
            if (r.max == null && x.length > 1)
              t.max = Math.max(t.max, x[x.length - 1]);
          }
          u = function (e) {
            var t = [],
              n,
          ...
          if (t.mode != "time" && r.tickDecimals == null) {
            var T = Math.max(0, -Math.floor(Math.log(s) / Math.LN10) + 1),
              N = u(t);
            if (!(N.length > 1 && /\..*0$/.test((N[1] - N[0]).toFixed(T))))
              t.tickDecimals = T;
          }
        }
      }
      t.tickGenerator = u;
      if (e.isFunction(r.tickFormatter))
    ...
    function K(t) {
      ...
      if (n == null || (typeof n == "number" && n > 0)) r = t.tickGenerator(t);
        ...
        if (e.isFunction(n)) r = n({ min: t.min, max: t.max });
        else r = n;
      }
      var i, s;
      t.ticks = [];
    ...
    function Q(e, t) {
      if (e.options.autoscaleMargin && t.length > 0) {
        ...
        if (e.options.max == null && t.length > 1)
    ...
    function et() {
      ...
      for (var f = 0; f < r.length; ++f) {
        ...
        h.strokeStyle =
          l.options.tickColor ||
          e.color.parse(l.options.color).scale("a", 0.22).toString();
        h.lineWidth = 1;
        if (l.direction == "x") {
          d = 0;
          if (p == "full") v = l.position == "top" ? 0 : w;
          else v = c.top - m.top + (l.position == "top" ? c.height : 0);
        } else {
          v = 0;
          if (p == "full") d = l.position == "left" ? 0 : b;
          else d = c.left - m.left + (l.position == "left" ? c.width : 0);
        }
        if (!l.innermost) {
          h.beginPath();
          g = y = 0;
          if (l.direction == "x") g = b;
          else y = w;
          if (h.lineWidth == 1) {
            d = Math.floor(d) + 0.5;
            v = Math.floor(v) + 0.5;
        ...
        for (t = 0; t < l.ticks.length; ++t) {
          ...
          if (h.lineWidth == 1) {
            if (l.direction == "x") d = Math.floor(d) + 0.5;
      ...
      for (var r = 0; r < n.length; ++r) {
        ...
        for (var o = 0; o < i.ticks.length; ++o) {
          ...
          if (i.direction == "x") {
            ...
            if (i.position == "bottom") a.top = s.top + s.padding;
            else a.bottom = y - (s.top + s.height - s.padding);
          } else {
    ...
    function it(e) {
      function t(e, t, n, r, i, s, o, u) {
        ...
        for (var l = 0; l < a.length; l += f) {
          ...
          p = o.p2c(p) + r;
          if (u == "circle")
            h.arc(c, p, t, 0, i ? Math.PI : Math.PI * 2, false);
          else u(h, c, p, t, i);
          h.closePath();
          if (n) {
            h.fillStyle = n;
    ...
    function at() {
      ...
      var l =
        ...
        n.join("") +
        "</table>";
      if (a.legend.container != null) e(a.legend.container).html(l);
      else {
        var c = "",
          h = a.legend.position,
          p = a.legend.margin;
        if (p[0] == null) p = [p, p];
        if (h.charAt(0) == "n") c += "top:" + (p[1] + m.top) + "px;";
        else if (h.charAt(0) == "s") c += "bottom:" + (p[1] + m.bottom) + "px;";
        if (h.charAt(1) == "e") c += "right:" + (p[0] + m.right) + "px;";
        else if (h.charAt(1) == "w") c += "left:" + (p[0] + m.left) + "px;";
        var d = e(
          '<div class="legend">' +
        ...
        if (a.legend.backgroundOpacity != 0) {
          ...
          if (v == null) {
            ...
            if (v && typeof v == "string") v = e.color.parse(v);
            else v = e.color.extract(d, "background-color");
            v.a = 1;
            v = v.toString();
          }
          var g = d.children();
      ...
      for (f = u.length - 1; f >= 0; --f) {
        ...
        if (c.lines.show || c.points.show) {
          for (l = 0; l < d.length; l += v) {
            ...
            var S = Math.abs(h.p2c(w) - e),
              x = Math.abs(p.p2c(E) - t),
              T = S * S + x * x;
            if (T < i) {
              i = T;
              s = [f, l / v];
        ...
        if (c.bars.show && !s) {
          var N = c.bars.align == "left" ? 0 : -c.bars.barWidth / 2,
            C = N + c.bars.barWidth;
          for (l = 0; l < d.length; l += v) {
            var w = d[l],
              E = d[l + 1],
    ...
    function gt() {
      ...
      for (e = 0; e < ft.length; ++e) {
        t = ft[e];
        if (t.series.bars.show) St(t.series, t.point);
        else Et(t.series, t.point);
      }
      p.restore();
      x(E.drawOverlay, [p]);
    }
    ...
    function Et(t, n) {
      ...
      p.lineWidth = u;
      p.strokeStyle = e.color.parse(t.color).scale("a", 0.5).toString();
      var a = 1.5 * u,
        r = s.p2c(r),
        i = o.p2c(i);
      p.beginPath();
      if (t.points.symbol == "circle") p.arc(r, i, a, 0, 2 * Math.PI, false);
      else t.points.symbol(p, r, i, a, false);
      p.closePath();
      p.stroke();
    }
    function St(t, n) {
      ...
      p.strokeStyle = e.color.parse(t.color).scale("a", 0.5).toString();
      var r = e.color.parse(t.color).scale("a", 0.5).toString();
